...
import uk.gov.hmrc.incorporatedentityidentificationfrontend.models.BusinessVerification
...
class BusinessVerificationControllerISpec extends ComponentSpecHelper with AuthStub
  ...
  "GET /business-verification-result" should {
    "redirect to /journey/redirect/:journeyId if BV status is stored successfully" in {
      ...
      stubstoreBusinessVerificationStatus(journeyId = testJourneyId,businessVerification = BusinessVerification("PASS") )(status=OK)
